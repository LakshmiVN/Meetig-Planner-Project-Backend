...
eventEmitter.on('alert-user-for-upcomming-event', (data) => {
  ...
  var mailOptions = {
    from: '"Meeting Planner" <vivekedwisor@gmail.com>',
    to: data.creatorForEmail,
    subject: `You have a meeting scheduled in 1 minute`,
    text: `Dear ${data.createdForName},
This is a reminder to let you know that your event "${data.eventTitle}" is occuring in 1 minute
Hope it helped you!`
  };
...
eventEmitter.on('sendAlertMail', (data) => {
  ...
  var mailOptions = {
    from: '"Meeting Planner" <vivekedwisor@gmail.com>',
    to: data.creatorForEmail,
    subject: `Your event has been ${data.actionPerformed}`,
    text: `Dear ${data.createdForName},
Your event with title "${data.eventTitle}" is ${data.actionPerformed} by ${data.creatorName}
Hope it helped you!`
  };
